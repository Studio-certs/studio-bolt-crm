import React, { useState, useEffect } from 'react';
    ...
    interface EditTemplateModalProps {
      template: Template;
      onClose: () => void;
      onSubmit: (templateId: string, updatedData: Partial<Template>) => Promise<void>;
    }
    ...
    export const EditTemplateModal: React.FC<EditTemplateModalProps> = ({
      ...
      // Update local state if the initial template prop changes (though unlikely in this flow)
      useEffect(() => {
        setTemplate({
          name: initialTemplate.name,
          type: initialTemplate.type,
      ...
      const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!template.name?.trim() || !template.type?.trim() || !template.prompt?.trim()) {
        ...
        try {
          await onSubmit(initialTemplate.id, template);
          // If onSubmit doesn't throw, the parent component will close the modal
        } catch (err) {
          setModalError(err instanceof Error ? err.message : 'Failed to update template.');
        } finally {
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 transform transition-all duration-300 scale-100 opacity-100">
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                <div>
                  <label htmlFor="template-name-edit" className="block text-sm font-medium text-gray-700 mb-1">
                    Name *
                  </label>
                  <input
                ...
                <div>
                  <label htmlFor="template-type-edit" className="block text-sm font-medium text-gray-700 mb-1">
                    Type *
                  </label>
                  <select
                  ...
                  >
                    ...
                    {/* Add other options here if they become available */}
                  </select>
                </div>

                <div>
                  <label htmlFor="template-prompt-edit" className="block text-sm font-medium text-gray-700 mb-1">
                    Prompt / Content *
                  </label>
                  <textarea
                    id="template-prompt-edit"
                    required
              ...
              <div className="mt-6 pt-4 border-t border-gray-200 flex justify-end space-x-3">
                ...
                <button
                  type="submit"
                  disabled={isSubmitting || !template.name?.trim() || !template.prompt?.trim()}
